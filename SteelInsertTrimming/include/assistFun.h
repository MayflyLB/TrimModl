...
void stdProcessCurves(vector<vector<CurveData*>>&temp_,tag_t spline=0);
void sortCurvesPointor(vector<CurveData*> tempDelete, vector<vector<CurveData*>> &rt_simplePro_,bool flag=false);
...
public: //原子重建
    ...
    void reEditLine()
    {
        if (UF_OBJ_ask_status(curve) != UF_OBJ_DELETED)
        {
            UF_CURVE_edit_line_data(curve, reinterpret_cast<UF_CURVE_line_p_t>(&start_point));
        }
        else
        {
            UF_CURVE_create_line(reinterpret_cast<UF_CURVE_line_p_t>(&start_point), &curve);
        }
        UF_VEC3_sub(end_point, start_point, dir_Center);
        UF_VEC3_distance(end_point, start_point, &dist_len);
    }
...
public:
    tag_t curve;
    Point3d start_point;//Param 0
    Point3d end_point;  //Param 1
    Point3d vertexPt;

    int type_; //UF_line_type UF_circle_type
    int baseType;//边70，否则0 
    Vector3d dir_Center;//直线 ->方向,圆弧->中心点坐标
